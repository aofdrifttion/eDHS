...
if (!function_exists('ensure_payment_history_table')) {
    function ensure_payment_history_table($conn) {
        ...
        try {
            ...
            // ตรวจสอบและเพิ่มคอลัมน์อัตโนมัติแบบ Defensive กรณีตารางเดิมของ รพ. อื่นสร้างไว้ไม่ครบ (เช่น ขาด accountcode หรือ bill_date)
            $existing_cols = [];
            $col_res = @mysqli_query($conn, "SHOW COLUMNS FROM `imr_tb_payment_history`");
            if ($col_res) {
            ...
            if (!empty($existing_cols)) {
                // 1. ตรวจสอบ patient_type
                if (!isset($existing_cols['patient_type'])) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `patient_type` ENUM('OPD','IPD') NOT NULL DEFAULT 'OPD'");
                    $existing_cols['patient_type'] = true;
                }
                // 2. ตรวจสอบ accountcode (แก้ปัญหา Unknown column 'accountcode' in 'INSERT INTO' ของ รพ. อื่น)
                if (!isset($existing_cols['accountcode'])) {
                    $after_acc = isset($existing_cols['patient_type']) ? " AFTER `patient_type`" : "";
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `accountcode` VARCHAR(50) DEFAULT NULL COMMENT 'รหัสผังบัญชี เช่น 1102050102.106'$after_acc");
                    $existing_cols['accountcode'] = true;
                }
                // 3. ตรวจสอบ pay_amount
                if (!isset($existing_cols['pay_amount'])) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `pay_amount` DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT 'ยอดเงินที่ชำระในงวดนี้'");
                    $existing_cols['pay_amount'] = true;
                }
                // 4. ตรวจสอบ bill_no
                if (!isset($existing_cols['bill_no'])) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `bill_no` VARCHAR(50) DEFAULT NULL COMMENT 'เลขที่ใบเสร็จ'");
                    $existing_cols['bill_no'] = true;
                }
                // 5. ตรวจสอบ bill_date
                if (!isset($existing_cols['bill_date'])) {
                    $after_bdate = isset($existing_cols['bill_no']) ? " AFTER `bill_no`" : "";
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `bill_date` DATE DEFAULT NULL COMMENT 'วันที่ตามใบเสร็จ'$after_bdate");
                    $existing_cols['bill_date'] = true;
                }
                // 6. ตรวจสอบ note
                if (!isset($existing_cols['note'])) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `note` TEXT DEFAULT NULL COMMENT 'หมายเหตุการแบ่งจ่าย'");
                    $existing_cols['note'] = true;
                }
                // 7. ตรวจสอบ record_date
                if (!isset($existing_cols['record_date'])) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD COLUMN `record_date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");
                    $existing_cols['record_date'] = true;
                }
                // 8. ตรวจสอบ Indexes
                $idx_res = @mysqli_query($conn, "SHOW INDEX FROM `imr_tb_payment_history` WHERE Key_name = 'idx_accountcode'");
                if ($idx_res && mysqli_num_rows($idx_res) == 0) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD INDEX `idx_accountcode` (`accountcode`)");
                }
                $idx_res_vn = @mysqli_query($conn, "SHOW INDEX FROM `imr_tb_payment_history` WHERE Key_name = 'idx_ref_vn_an'");
                if ($idx_res_vn && mysqli_num_rows($idx_res_vn) == 0) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_payment_history` ADD INDEX `idx_ref_vn_an` (`ref_vn_an`)");
                }
            }
