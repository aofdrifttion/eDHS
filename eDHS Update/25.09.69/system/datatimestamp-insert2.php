...
// รับประกันว่าตารางประวัติการแบ่งจ่าย (imr_tb_payment_history) มีอยู่เสมอ (Zero-Config Auto-Migration)
if (function_exists('ensure_payment_history_table')) {
    ensure_payment_history_table($conn);
}
...
if (!function_exists('getSourceShort')) {
    function getSourceShort($source) {
        ...
        if (isset($_POST['action_delete_partial'])) {
            ...
            // รับค่าตัวแปร
            $id = mysqli_real_escape_string($conn, (string)($_POST['id'] ?? ''));
            $vn = mysqli_real_escape_string($conn, (string)($_POST['vn'] ?? ''));
            $type = mysqli_real_escape_string($conn, (string)($_POST['type'] ?? 'OPD'));
            ...
            try {
                ...
                $total_paid = ($row_sum && $row_sum['total']) ? floatval(cleanNum($row_sum['total'])) : 0.0;

                // ตรวจสอบยอดหนี้ตั้งต้น
                $chk_debt = mysqli_query($conn, "SELECT debit FROM $main_table WHERE $pk_col = '$vn'");
                $row_debt = ($chk_debt) ? mysqli_fetch_assoc($chk_debt) : null;
                $debit_val = ($row_debt && isset($row_debt['debit'])) ? floatval(cleanNum($row_debt['debit'])) : 0.0;
                ...
                if ($last_receipt_q && $lr = mysqli_fetch_assoc($last_receipt_q)) {
                    $last_bill_no = mysqli_real_escape_string($conn, (string)$lr['bill_no']);
                    if (!empty($lr['bill_date']) && $lr['bill_date'] !== '0000-00-00') {
                        $ts_lr = strtotime($lr['bill_date']);
                        if ($ts_lr) {
                            $d_lr = date('d', $ts_lr);
                            $m_lr = date('m', $ts_lr);
                            $y_lr = (int)date('Y', $ts_lr) + 543;
                ...
                error_log("delete_partial error: " . $e->getMessage());
                echo "error_delete: " . $e->getMessage();
            }
        ...
        if (isset($_POST['action_save_partial'])) {
            ...
            try {
                $vn = mysqli_real_escape_string($conn, (string)($_POST['vn'] ?? ''));
                $amount = str_replace(',', '', (string)($_POST['amount'] ?? '0')); // เอาลูกน้ำออก
                $bill_no = mysqli_real_escape_string($conn, format_debtor_bill_no($_POST['bill_no'] ?? ''));
                $type = mysqli_real_escape_string($conn, (string)($_POST['type'] ?? 'OPD')); // OPD หรือ IPD
                $note = isset($_POST['note']) ? mysqli_real_escape_string($conn, (string)$_POST['note']) : '';
                $target_accountcode = isset($_POST['accountcode']) ? mysqli_real_escape_string($conn, (string)$_POST['accountcode']) : '';

                // รับค่า ID สำหรับกรณีแก้ไข (ถ้ามี)
                $history_id = isset($_POST['history_id']) ? mysqli_real_escape_string($conn, (string)$_POST['history_id']) : '';

                // แปลงวันที่ไทย/สากลเป็น Y-m-d เพื่อเก็บลงฐานข้อมูล (รองรับทั้ง / และ - ป้องกัน Strict Mode Crash)
                $bill_date_sql = function_exists('parse_bill_date_to_sql') 
                    ? parse_bill_date_to_sql($_POST['bill_date'] ?? '') 
                    : null;
                $bill_date_clause = ($bill_date_sql !== null) 
                    ? "'" . mysqli_real_escape_string($conn, $bill_date_sql) . "'" 
                    : "NULL";
                ...
                if (!empty($history_id)) {
                    ...
                    if ($chk_history && mysqli_num_rows($chk_history) == 0) {
                        $chk_main = mysqli_query($conn, "SELECT follow_money, bill FROM $main_table WHERE $pk_col = '$vn'");
                        if ($chk_main && $row_main = mysqli_fetch_assoc($chk_main)) {
                            $legacy_money = isset($row_main['follow_money']) ? floatval(cleanNum($row_main['follow_money'])) : 0.0;

                            if ($legacy_money > 0) {
                                 $old_bill = !empty($row_main['bill']) ? mysqli_real_escape_string($conn, (string)$row_main['bill']) : 'ยอดยกมา';
                    ...
                    // 2. คำสั่ง Insert รายการใหม่ (ใช้ $bill_date_clause เพื่อส่ง NULL อย่างถูกต้องใน Strict Mode)
                    $sql_action = "INSERT INTO imr_tb_payment_history
                                   (ref_vn_an, patient_type, accountcode, pay_amount, bill_no, bill_date, note)
                                   VALUES ('$vn', '$type', '$target_accountcode', '$amount', '$bill_no', $bill_date_clause, '$note')";
                }
                ...
                if (mysqli_query($conn, $sql_action)) {
                    ...
                    if ($is_cr_child) {
                        ...
                        $row_sum_cr = ($res_sum_cr) ? mysqli_fetch_assoc($res_sum_cr) : null;
                        $cr_paid = ($row_sum_cr && $row_sum_cr['total']) ? floatval(cleanNum($row_sum_cr['total'])) : $total_paid;
                        ...
                        $row_sum_sss = ($res_sum_sss) ? mysqli_fetch_assoc($res_sum_sss) : null;
                        $sss_paid = ($row_sum_sss && $row_sum_sss['total']) ? floatval(cleanNum($row_sum_sss['total'])) : $total_paid;
                ...
                error_log("save_partial error: " . $e->getMessage());
...
if (isset($_POST['action'])) {
    ...
    foreach ($months_to_process as $m_txt) {
        ...
        if (is_sss_effective_for_month($sss_config, $m_txt) && $auto_split_sss) {
            $vn_filter_sss = null;
            if ($has_date_range) {
                $vns_sss_q = mysqli_query($conn, "SELECT vn FROM imr_tb_debtor_rights_opd WHERE monthtxt = '$m_txt' AND $date_condition AND accountcode IN ('1102050101.301','1102050101.303','1102050101.307','1102050101.309')");
                $vn_filter_sss = [];
                if ($vns_sss_q) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // ปรับยอด SSS Instrument Cross-Account Splitting (ลดผังแม่ .301, .303, .307 และเพิ่มผัง .309)
...
if (isset($_POST['action2'])) {
    ...
    foreach ($months_to_process_ipd as $m_txt_i) {
        if (is_cr_effective_for_month($cr_config_ipd, $m_txt_i) && $auto_split_ipd) {
            ...
            if (!$has_date_range_ipd || !empty($an_filter_cr)) {
                ...
                if ($s_cr_i && !empty($s_cr_i['auto_split'])) {
                    $has_any_cr_ipd = true;
        ...
        if (is_sss_effective_for_month($sss_config_ipd, $m_txt_i) && $auto_split_sss_ipd) {
            $an_filter_sss = null;
            if ($has_date_range_ipd) {
                $ans_sss_q = mysqli_query($conn, "SELECT an FROM imr_tb_debtor_rights_ipd WHERE monthtxt = '$m_txt_i' AND $date_condition_plain_i AND accountcode IN ('1102050101.302', '1102050101.304', '1102050101.308', '1102050101.310')");
                $an_filter_sss = [];
                if ($ans_sss_q) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($cr_summary_ipd) {
                if (isset($cr_summary_ipd['transfers_by_account'][$row['accountcode']])) {
                    ...
                    $rendered_217 = true;
                    $row["debit"] = cleanNum($cr_summary_ipd['cr_non_kidney_received_total']);
            ...
            // ปรับยอด SSS Instrument Cross-Account Splitting (ลดผังแม่ .302, .304, .308 และเพิ่มผัง .310)
            if ($sss_summary_ipd) {
                if (isset($sss_summary_ipd['transfers_by_account'][$row['accountcode']])) {
                    ...
                    $row["can"] = $sss_summary_ipd['sss_count'];
    ...
    if (isset($row['tel'])) $row['tel'] = decrypt_data($row['tel']);
        ...
        if(isset($_POST['stmgeto'])){
            ...
            foreach ($months_to_process_opd as $m_stm_txt) {
                ...
                if (is_sss_effective_for_month($sss_config, $m_stm_txt) && $auto_split_sss_opd) {
                    ...
                    $vn_filter_sss = null;
                    if ($has_date_range_opd) {
                        $vns_sss_q = mysqli_query($conn, "SELECT vn FROM imr_tb_debtor_rights_opd o WHERE monthtxt = '$m_stm_txt' AND $date_condition_stm_o AND accountcode IN ('1102050101.301','1102050101.303','1102050101.307','1102050101.309')");
                        $vn_filter_sss = [];
                        if ($vns_sss_q) {
                            while ($vr = mysqli_fetch_assoc($vns_sss_q)) {
                                $vn_filter_sss[] = $vr['vn'];
            ...
            $is_sss_parent_opd = $is_sss_effective && $auto_split_sss_opd && in_array(explode('_', $stmget)[0], ['1102050101.301', '1102050101.303', '1102050101.307']) && is_sss_parent_account_enabled($sss_config, explode('_', $stmget)[0]) && !$is_kidney_click && !$is_hospmain_click;
            ...
            if (!empty($vns_to_detect)) {
                ...
                $q_cr_bd = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND vn IN ($safe_det_vns) ORDER BY CASE WHEN bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-' THEN 1 ELSE 2 END ASC, CASE WHEN compensated IS NOT NULL AND compensated > 0 THEN 1 ELSE 2 END ASC, id DESC");
                if ($q_cr_bd) {
                ...
                $q_sss_bd = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn IN ($safe_det_vns) ORDER BY CASE WHEN bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-' THEN 1 ELSE 2 END ASC, CASE WHEN compensated IS NOT NULL AND compensated > 0 THEN 1 ELSE 2 END ASC, id DESC");
                if ($q_sss_bd) {
                ...
                if ($is_target_216 && !empty($all_cr_visits)) {
                    ...
                    if ($is_transferred_visit) {
                        ...
                        if ($vinfo['is_full_transfer']) {
                            ...
                            // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                            $child_cr_bill = trim($cr_bd_map[$vn]['bill'] ?? ($vinfo['bill'] ?? ''));
                            $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                            if (!empty($child_cr_bill) && trim($row['bill']) === $child_cr_bill && !$is_mother_debt_settled) {
                                $row['bill'] = '';
                                $row['billdate'] = '';
                                if (isset($row['bookno']) && trim($row['bookno']) === $child_cr_bill) {
                                    $row['bookno'] = '';
                                }
                            }
                            $all_display_rows[] = $row;
                    ...
                    if ($is_sss_transferred_visit) {
                        ...
                        if ($vinfo['is_full_transfer']) {
                            ...
                            // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                            $child_sss_bill = trim($sss_bd_map[$vn]['bill'] ?? ($vinfo['bill'] ?? ''));
                            $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                            if (!empty($child_sss_bill) && trim($row['bill']) === $child_sss_bill && !$is_mother_debt_settled) {
                                $row['bill'] = '';
                                $row['billdate'] = '';
                                if (isset($row['bookno']) && trim($row['bookno']) === $child_sss_bill) {
                                    $row['bookno'] = '';
                                }
                            }
                            $all_display_rows[] = $row;
        ...
        if(isset($_POST['stmgeti'])){
            ...
            foreach ($months_to_process_ipd as $m_stm_i) {
                ...
                if (is_sss_effective_for_month($sss_config, $m_stm_i) && $auto_split_sss_ipd) {
                    ...
                    $an_filter_sss = null;
                    if ($has_date_range_ipd) {
                        $ans_s_q = mysqli_query($conn, "SELECT an FROM imr_tb_debtor_rights_ipd o WHERE monthtxt = '$m_stm_i' AND $date_condition_stm_i AND accountcode IN ('1102050101.302', '1102050101.304', '1102050101.308', '1102050101.310')");
                        $an_filter_sss = [];
                        if ($ans_s_q) {
            ...
            $is_sss_parent_ipd = $is_sss_effective_ipd && $auto_split_sss_ipd && in_array($base_stmget, ['1102050101.302', '1102050101.304', '1102050101.308']) && is_sss_parent_account_enabled($sss_config, $base_stmget) && !$is_kidney_click_ipd;
            ...
            if (!empty($ans_to_detect)) {
                ...
                $q_cr_bd_i = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'IPD' AND vn IN ($safe_det_ans) ORDER BY CASE WHEN bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-' THEN 1 ELSE 2 END ASC, CASE WHEN compensated IS NOT NULL AND compensated > 0 THEN 1 ELSE 2 END ASC, id DESC");
                if ($q_cr_bd_i) {
                    while ($bd = mysqli_fetch_assoc($q_cr_bd_i)) {
                        $v = $bd['vn'];
                        if (!isset($cr_bd_map_i[$v]) || (!empty($bd['bill']) && empty($cr_bd_map_i[$v]['bill']))) {
                            $cr_bd_map_i[$v] = $bd;
                        }
                    }
                }
                $q_sss_bd_i = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn IN ($safe_det_ans) ORDER BY CASE WHEN bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-' THEN 1 ELSE 2 END ASC, CASE WHEN compensated IS NOT NULL AND compensated > 0 THEN 1 ELSE 2 END ASC, id DESC");
                if ($q_sss_bd_i) {
                    while ($bd = mysqli_fetch_assoc($q_sss_bd_i)) {
                        $v = $bd['vn'];
                        if (!isset($sss_bd_map_i[$v]) || (!empty($bd['bill']) && empty($sss_bd_map_i[$v]['bill']))) {
                            $sss_bd_map_i[$v] = $bd;
                        }
                    }
                }
                ...
                if ($is_cr_target_217 && !empty($all_cr_visits_ipd)) {
                    ...
                    if ($is_transferred_visit) {
                        ...
                        if ($vinfo['is_full_transfer']) {
                            ...
                            // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                            $child_cr_bill = trim($cr_bd_map_i[$an]['bill'] ?? ($vinfo['bill'] ?? ''));
                            $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                            if (!empty($child_cr_bill) && trim($row['bill']) === $child_cr_bill && !$is_mother_debt_settled) {
                                $row['bill'] = '';
                                $row['billdate'] = '';
                    ...
                    if ($is_sss_transferred_visit) {
                        ...
                        if ($vinfo['is_full_transfer']) {
                            ...
                            // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                            $child_sss_bill = trim($sss_bd_map_i[$an]['bill'] ?? ($vinfo['bill'] ?? ''));
                            $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                            if (!empty($child_sss_bill) && trim($row['bill']) === $child_sss_bill && !$is_mother_debt_settled) {
                                $row['bill'] = '';
                                $row['billdate'] = '';
        ...
        if(isset($_POST['actionstmvlookup'])){
          ...
          WHERE (UPPER(TRIM(stm.fund)) = 'UCS' OR (stm.compensated != '0' AND stm.compensated != '0.0' AND stm.compensated != '0.00'))
          GROUP BY stm.rep
    ...
    if(isset($_POST['stmgetovlookup'])){
          ...
          $sql = "SELECT stm.yearbudget,stm.rep,o.vn,o.ptname,o.debit,o.follow_money AS follow_money_o,i.an,i.ptname as 'ptnamei',i.debit as 'debiti',i.follow_money AS follow_money_i,stm.compensated,(stm.compensated)-(o.debit) AS 'diff',(stm.compensated)-(i.debit) AS 'diffi',stm.accountname,stm.fund,o.vstdate,i.admdate,i.dchdate,stm.accountcode,i.mobile as 'mobilei',o.mobile as 'mobileo',o.cid as cido,i.cid as cidi, o.bill AS billo, i.bill AS billi
            ...
            LEFT JOIN  imr_tb_debtor_rights_opd o on o.vn=stm.vn
            LEFT JOIN  imr_tb_debtor_rights_ipd i on i.an=stm.vn
            WHERE stm.rep = '$stmgetovlookup' AND (UPPER(TRIM(stm.fund)) = 'UCS' OR (stm.compensated != '0' AND stm.compensated != '0.0' AND stm.compensated != '0.00'))
            ORDER BY diff desc ";
    ...
    if (isset($d['tel'])) $d['tel'] = decrypt_data($d['tel']);
          ...
          foreach ($all_rows as $row) {
            ...
            $diffi = $diffi + cleanNum($row["diffi"]);
            ...
            $total_debit = $debit + $debiti;
            $total_diff = $diff + $diffi;

            echo '<tfoot>
              <tr class="summary-total-row">
                <td style="text-align:center;font-weight: bold;" colspan="9">รวม</td>
                <td style="text-align: right;font-weight: bold;" id="o1">'.formatMoney($total_debit, 2).'</td>
                <td style="text-align: right;font-weight: bold;" id="o2">'.formatMoney($compensated, 2).'</td>
                <td style="text-align: right;font-weight: bold;" id="o3">'.formatMoney($total_diff, 2).'</td>
              </tr>
            </tfoot>';
...
if(isset($_POST['actionopdlistonly'])){
  ...
  $_SESSION['oid'] = $id; // บันทึกค่าเต็มลงเซสชันเพื่อให้หน้าพิมพ์รายงานนำไปดักกรองต่อได้อย่างแม่นยำ
    ...
    $is_sss_parent_opd = in_array($id, ['1102050101.301', '1102050101.303', '1102050101.307']);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $is_sss_target_account_opd = $is_sss_effective && $auto_split_sss_opd && !$is_kidney_click && (
            $is_target_309 ||
            (in_array($base_id, ['1102050101.301', '1102050101.303', '1102050101.307']) && is_sss_parent_account_enabled($sss_config, $base_id))
        );
...
// กรณีมีการแบ่งจ่ายชำระหนี้: ให้ตรวจสอบยอดหนี้คงเหลือจริง
                     ...
                     if ($balance_check <= 0.01) {
                         $show_bill_main = format_debtor_bill_no($row["bill"]);
                ...
                if (mysqli_num_rows($result1) > 0) {
                    while ($row1 = mysqli_fetch_assoc($result1)) {
                     ...
                     $vn_c3       = $row["vn"];
                     $c3_bill     = format_debtor_bill_no($row["bill"]);     // ค่าเริ่มต้น (อาจว่าง)
                     ...
                     if (cleanNum($row["follow_money"]) > 0 && $c3_balance <= 0.01) {
                         ...
                         if ($res_c3 !== false && ($row_c3 = mysqli_fetch_assoc($res_c3))) {
                             $c3_bill = format_debtor_bill_no($row_c3['bill_no']);
                             if (!empty($row_c3['bill_date']) && $row_c3['bill_date'] !== '0000-00-00') {
                                 $c3_billdate = function_exists('format_sql_date_to_th') 
                                     ? format_sql_date_to_th($row_c3['bill_date']) 
                                     : date('d/m/', strtotime($row_c3['bill_date'])) . ((int)date('Y', strtotime($row_c3['bill_date'])) + 543);
                             }
            ...
            if (mysqli_num_rows($result_sub) > 0) {
                ...
                while ($sub = mysqli_fetch_assoc($result_sub)) {
                  ...
                  $sub_date_show = "-";
                  if (!empty($sub['bill_date']) && $sub['bill_date'] !== '0000-00-00') {
                      $sub_date_show = function_exists('format_sql_date_to_th') 
                          ? format_sql_date_to_th($sub['bill_date']) 
                          : date('d/m/', strtotime($sub['bill_date'])) . ((int)date('Y', strtotime($sub['bill_date'])) + 543);
                  }
                  ...
                  echo '<td style="text-align: center;">'.format_debtor_bill_no($sub['bill_no']).'</td>';
                  ...
                  echo '<td style="text-align: center;background-color: #fff;">
                          <a href="javascript:void(0)" class="btn-edit-partial"
                             ...
                             data-bill="'.htmlspecialchars(format_debtor_bill_no($sub['bill_no']), ENT_QUOTES, 'UTF-8').'"
...
if(isset($_POST['actionipdlistonly'])){
  ...
  $is_sss_parent_ipd = in_array($id, ['1102050101.302', '1102050101.304', '1102050101.308']);
  ...
  if ($result && mysqli_num_rows($result) > 0) {
    ...
    $is_sss_target_account_ipd = $is_sss_effective_ipd && $auto_split_sss_ipd && !$is_kidney_click && (
        $is_target_310 ||
        (in_array($base_id, ['1102050101.302', '1102050101.304', '1102050101.308']) && is_sss_parent_account_enabled($sss_config_ipd, $base_id))
    );
    ...
    if (!empty($ans_to_detect)) {
        ...
        if ($q_cr_bd_i) {
            while ($bd = mysqli_fetch_assoc($q_cr_bd_i)) {
                $v = $bd['vn'];
                if (!isset($cr_bd_map_ipd[$v]) || (!empty($bd['bill']) && empty($cr_bd_map_ipd[$v]['bill']))) {
                    $cr_bd_map_ipd[$v] = $bd;
                }
            }
        }
        ...
        if ($q_sss_bd_i) {
            while ($bd = mysqli_fetch_assoc($q_sss_bd_i)) {
                $v = $bd['vn'];
                if (!isset($sss_bd_map_ipd[$v]) || (!empty($bd['bill']) && empty($sss_bd_map_ipd[$v]['bill']))) {
                    $sss_bd_map_ipd[$v] = $bd;
                }
            }
        }
        ...
        if ($is_target_217 && !empty($all_cr_visits_ipd)) {
            ...
            if ($is_transferred_visit) {
                ...
                if ($vinfo['is_full_transfer']) {
                    ...
                    // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                    $child_cr_bill = trim($cr_bd_map_ipd[$an]['bill'] ?? ($vinfo['bill'] ?? ''));
                    $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                    if (!empty($child_cr_bill) && trim($row['bill']) === $child_cr_bill && !$is_mother_debt_settled) {
                        $row['bill'] = '';
                        $row['billdate'] = '';
            ...
            if ($is_sss_transferred_visit) {
                ...
                if ($vinfo['is_full_transfer']) {
                    ...
                    // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                    $child_sss_bill = trim($sss_bd_map_ipd[$an]['bill'] ?? ($vinfo['bill'] ?? ''));
                    $is_mother_debt_settled = ($mother_comp > 0 && ($row['debit'] - $mother_comp) <= 0.01);
                    if (!empty($child_sss_bill) && trim($row['bill']) === $child_sss_bill && !$is_mother_debt_settled) {
                        $row['bill'] = '';
                        $row['billdate'] = '';
...
if(isset($_POST['follow_money'])){
  ...
  if ($is_cr_child) {
      $q_bd = mysqli_query($conn, "SELECT general_remain_amount FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND vn = '$vn' LIMIT 1");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown 
                   ...
                   WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['general_remain_amount']) <= 0) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET follow_money = '$data' WHERE vn = '$vn'");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.216 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET follow_money = '$data' WHERE vn = '$vn'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT general_remain_amount FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn = '$vn' LIMIT 1");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown 
                   SET compensated = '$clean_amt',
                       settle_status = (CASE WHEN '$clean_amt' >= item_amount THEN 'SETTLED' WHEN '$clean_amt' > 0 THEN 'PARTIAL' ELSE 'WAIT' END)
                   WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['general_remain_amount']) <= 0) {
      ...
      $child_comp = 0;
      $q_cr = mysqli_query($conn, "SELECT compensated FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND vn = '$vn' LIMIT 1");
      if ($q_cr && $r_cr = mysqli_fetch_assoc($q_cr)) {
          $child_comp += floatval(cleanNum($r_cr['compensated']));
      }
      $q_sss = mysqli_query($conn, "SELECT compensated FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn = '$vn' LIMIT 1");
      if ($q_sss && $r_sss = mysqli_fetch_assoc($q_sss)) {
          $child_comp += floatval(cleanNum($r_sss['compensated']));
      }
      $total_main_money = floatval(cleanNum($data)) + $child_comp;
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET follow_money = '$total_main_money' WHERE vn='$vn'");
  }
...
if(isset($_POST['bill'])){
  ...
  $vn= mysqli_real_escape_string($conn, $_POST['follow_moneyvn']);
  $data= mysqli_real_escape_string($conn, format_debtor_bill_no($_POST['bill']));
  $typecc= mysqli_real_escape_string($conn, $_POST['typecc'] ?? 'OPD');
  $target_accountcode = isset($_POST['accountcode']) ? mysqli_real_escape_string($conn, $_POST['accountcode']) : '';
  ...
  if ($is_cr_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown 
                   SET bill = '$data', settle_status = '$status'
                   WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET bill = '$data' WHERE vn = '$vn' AND (accountcode LIKE '%1102050101.216%' OR debit <= 0)");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.216 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET bill = '$data' WHERE vn = '$vn' AND accountcode LIKE '%1102050101.216%'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown 
                   SET bill = '$data', settle_status = '$status'
                   WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET bill = '$data' WHERE vn = '$vn' AND (accountcode LIKE '%1102050101.309%' OR debit <= 0)");
          }
      } else {
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET bill = '$data' WHERE vn = '$vn' AND accountcode LIKE '%1102050101.309%'");
      }
  } else {
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET bill = '$data' WHERE vn='$vn'");
  }
...
if(isset($_POST['billdate'])){
  ...
  if ($is_cr_child) {
      ...
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown SET billdate = '$data' WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET billdate = '$data' WHERE vn = '$vn' AND (accountcode LIKE '%1102050101.216%' OR debit <= 0)");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.216 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET billdate = '$data' WHERE vn = '$vn' AND accountcode LIKE '%1102050101.216%'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown SET billdate = '$data' WHERE visit_type = 'OPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET billdate = '$data' WHERE vn = '$vn' AND (accountcode LIKE '%1102050101.309%' OR debit <= 0)");
          }
      } else {
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET billdate = '$data' WHERE vn = '$vn' AND accountcode LIKE '%1102050101.309%'");
      }
  } else {
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd SET billdate = '$data' WHERE vn='$vn'");
  }
...
if(isset($_POST['follow_money_i'])){
  ...
  if ($is_cr_child) {
      $q_bd = mysqli_query($conn, "SELECT general_remain_amount FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'IPD' AND vn = '$vn' LIMIT 1");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown 
                   SET compensated = '$clean_amt',
                       settle_status = (CASE WHEN '$clean_amt' >= item_amount THEN 'SETTLED' WHEN '$clean_amt' > 0 THEN 'PARTIAL' ELSE 'WAIT' END)
                   WHERE visit_type = 'IPD' AND vn = '$vn'");
          if (floatval($r_bd['general_remain_amount']) <= 0) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET follow_money = '$data' WHERE an = '$vn'");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.217 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET follow_money = '$data' WHERE an = '$vn'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT general_remain_amount FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn = '$vn' LIMIT 1");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown 
                   SET compensated = '$clean_amt',
      ...
      $child_comp = 0;
      $q_cr = mysqli_query($conn, "SELECT compensated FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'IPD' AND vn = '$vn' LIMIT 1");
      if ($q_cr && $r_cr = mysqli_fetch_assoc($q_cr)) {
          $child_comp += floatval(cleanNum($r_cr['compensated']));
      }
      $q_sss = mysqli_query($conn, "SELECT compensated FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn = '$vn' LIMIT 1");
      if ($q_sss && $r_sss = mysqli_fetch_assoc($q_sss)) {
          $child_comp += floatval(cleanNum($r_sss['compensated']));
      }
      $total_main_money = floatval(cleanNum($data)) + $child_comp;
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET follow_money = '$total_main_money' WHERE an='$vn'");
  }
...
if(isset($_POST['bill_i'])){
  ...
  $vn= mysqli_real_escape_string($conn, $_POST['follow_moneyvn']);
  $data= mysqli_real_escape_string($conn, format_debtor_bill_no($_POST['bill_i']));
  $typecc= mysqli_real_escape_string($conn, $_POST['typecc'] ?? 'IPD');
  $target_accountcode = isset($_POST['accountcode']) ? mysqli_real_escape_string($conn, $_POST['accountcode']) : '';
  ...
  if ($is_cr_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'IPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown 
                   SET bill = '$data', settle_status = '$status'
                   WHERE visit_type = 'IPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET bill = '$data' WHERE an = '$vn' AND (accountcode LIKE '%1102050101.217%' OR debit <= 0)");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.217 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET bill = '$data' WHERE an = '$vn' AND accountcode LIKE '%1102050101.217%'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown 
                   SET bill = '$data', settle_status = '$status'
                   WHERE visit_type = 'IPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET bill = '$data' WHERE an = '$vn' AND (accountcode LIKE '%1102050101.310%' OR debit <= 0)");
          }
      } else {
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET bill = '$data' WHERE an = '$vn' AND accountcode LIKE '%1102050101.310%'");
      }
  } else {
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET bill = '$data' WHERE an='$vn'");
  }
...
if(isset($_POST['billdate_i'])){
  ...
  if ($is_cr_child) {
      ...
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_cr_breakdown SET billdate = '$data' WHERE visit_type = 'IPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET billdate = '$data' WHERE an = '$vn' AND (accountcode LIKE '%1102050101.217%' OR debit <= 0)");
          }
      } else {
          // รายการที่เป็น Native ใน 1102050101.217 หรือเดือนที่ไม่ได้เปิด CR Splitting
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET billdate = '$data' WHERE an = '$vn' AND accountcode LIKE '%1102050101.217%'");
      }
  } elseif ($is_sss_child) {
      $q_bd = mysqli_query($conn, "SELECT MAX(general_remain_amount) as max_remain FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn = '$vn'");
      if ($q_bd && mysqli_num_rows($q_bd) > 0) {
          $r_bd = mysqli_fetch_assoc($q_bd);
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_sss_breakdown SET billdate = '$data' WHERE visit_type = 'IPD' AND vn = '$vn'");
          if (floatval($r_bd['max_remain'] ?? 0) <= 0.01) {
              mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET billdate = '$data' WHERE an = '$vn' AND (accountcode LIKE '%1102050101.310%' OR debit <= 0)");
          }
      } else {
          $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET billdate = '$data' WHERE an = '$vn' AND accountcode LIKE '%1102050101.310%'");
      }
  } else {
      $success = mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd SET billdate = '$data' WHERE an='$vn'");
  }
      ...
      while ($row = mysqli_fetch_assoc($result)) {
          ...
          if (!empty($all_sss_summaries) && !$is_kidney) {
              if ($accountcode === '1102050101.309') {
                  ...
                  $row["incomediff"] = 0.0;
                  $row["visitall"] = $total_sss_309_count;
                  $row["visitdiff"] = $total_sss_309_count;
      ...
      while ($row = mysqli_fetch_assoc($result)) {
          ...
          if (!empty($all_sss_summaries_ipd) && !$is_kidney) {
              if ($accountcode === '1102050101.310') {
                  ...
                  $row["incomediff"] = 0.0;
                  $row["visitall"] = $total_sss_310_count;
                  $row["visitdiff"] = $total_sss_310_count;
    ...
    if ($should_run_query) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            foreach ($all_display_rows as $row) {
                ...
                $bill_no = format_debtor_bill_no($bill_no);
...
if (isset($_POST['action_bulk_update_bill'])) {
    ...
    $bill = mysqli_real_escape_string($conn, format_debtor_bill_no($_POST['bulk_bill']));         // รับค่าจากชื่อใหม่
    ...
    if ($is_cr_child) {
        ...
        // สำหรับเคสที่โอน 100% (ไม่มีหนี้คงเหลือที่ผังแม่ และยอด debit ในตารางหลัก <= 0) ให้อัปเดตตารางหลักด้วย
        if ($typecc === 'OPD') {
            // 1. อัปเดตตารางหลักสำหรับรายการ Native ใน 1102050101.216 หรือรายการที่ไม่มีใน breakdown
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd 
                SET bill = '$bill', billdate = '$billdate' 
                WHERE vn IN ($id_string) AND (accountcode LIKE '%1102050101.216%' OR vn NOT IN (SELECT vn FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND vn IN ($id_string)))");

            // 2. สำหรับรายการที่โอนมาจากผังแม่ 100% (ไม่มีหนี้คงเหลือที่ผังแม่ และตารางหลักมียอด debit <= 0) ให้อัปเดตตารางหลัก
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd o
                SET o.bill = '$bill', o.billdate = '$billdate' 
                WHERE o.vn IN ($id_string) 
                  AND o.accountcode NOT LIKE '%1102050101.216%'
                  AND (o.debit <= 0 OR o.debit IS NULL)
                  AND NOT EXISTS (
                      SELECT 1 FROM imr_tb_debtor_cr_breakdown b 
                      WHERE b.vn = o.vn AND b.visit_type = 'OPD' AND b.general_remain_amount > 0.01
                  )");
        } else {
            // 1. อัปเดตตารางหลักสำหรับรายการ Native ใน 1102050101.217 หรือรายการที่ไม่มีใน breakdown
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd 
                SET bill = '$bill', billdate = '$billdate' 
                WHERE an IN ($id_string) AND (accountcode LIKE '%1102050101.217%' OR an NOT IN (SELECT vn FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'IPD' AND vn IN ($id_string)))");

            // 2. สำหรับรายการที่โอนมาจากผังแม่ 100% (ไม่มีหนี้คงเหลือที่ผังแม่ และตารางหลักมียอด debit <= 0) ให้อัปเดตตารางหลัก
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd o
                SET o.bill = '$bill', o.billdate = '$billdate' 
                WHERE o.an IN ($id_string) 
                  AND o.accountcode NOT LIKE '%1102050101.217%'
                  AND (o.debit <= 0 OR o.debit IS NULL)
                  AND NOT EXISTS (
                      SELECT 1 FROM imr_tb_debtor_cr_breakdown b 
                      WHERE b.vn = o.an AND b.visit_type = 'IPD' AND b.general_remain_amount > 0.01
                  )");
        }
    } elseif ($is_sss_child) {
        ...
        if ($typecc === 'OPD') {
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd 
                SET bill = '$bill', billdate = '$billdate' 
                WHERE vn IN ($id_string) AND (accountcode LIKE '%1102050101.309%' OR vn NOT IN (SELECT vn FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND vn IN ($id_string)))");

            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_opd o
                SET o.bill = '$bill', o.billdate = '$billdate' 
                WHERE o.vn IN ($id_string) 
                  AND o.accountcode NOT LIKE '%1102050101.309%'
                  AND (o.debit <= 0 OR o.debit IS NULL)
                  AND NOT EXISTS (
                      SELECT 1 FROM imr_tb_debtor_sss_breakdown b 
                      WHERE b.vn = o.vn AND b.visit_type = 'OPD' AND b.general_remain_amount > 0.01
                  )");
        } else {
            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd 
                SET bill = '$bill', billdate = '$billdate' 
                WHERE an IN ($id_string) AND (accountcode LIKE '%1102050101.310%' OR an NOT IN (SELECT vn FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'IPD' AND vn IN ($id_string)))");

            mysqli_query($conn, "UPDATE imr_tb_debtor_rights_ipd o
                SET o.bill = '$bill', o.billdate = '$billdate' 
                WHERE o.an IN ($id_string) 
                  AND o.accountcode NOT LIKE '%1102050101.310%'
                  AND (o.debit <= 0 OR o.debit IS NULL)
                  AND NOT EXISTS (
                      SELECT 1 FROM imr_tb_debtor_sss_breakdown b 
                      WHERE b.vn = o.an AND b.visit_type = 'IPD' AND b.general_remain_amount > 0.01
