...
if ($action === 'get_month_audit_summary') {
    ...
    // ปรับปรุงยอด batch_excel ด้วยการตัดโอน CR & SSS ของเดือนนี้
    if ($cr_opd) {
        ...
        foreach ($cr_opd['transfers_by_account'] as $acc => $tr) {
            ...
            $batch_excel['1102050101.216']['total_debit'] += $tr['transfer_out'];
            $batch_excel['1102050101.216']['count'] += $tr['full_transfer_cases'];
        }
    }
    if ($cr_ipd) {
        ...
        foreach ($cr_ipd['transfers_by_account'] as $acc => $tr) {
            ...
            $batch_excel['1102050101.217']['total_debit'] += $tr['transfer_out'];
            $batch_excel['1102050101.217']['count'] += ($tr['full_transfer_cases'] ?? 0);
        }
    }
    if ($sss_opd) {
        ...
        foreach ($sss_opd['transfers_by_account'] as $acc => $tr) {
            ...
            $batch_excel['1102050101.309']['total_debit'] += $tr['transfer_out'];
            $batch_excel['1102050101.309']['count'] += $tr['full_transfer_cases'];
        }
    }
    if ($sss_ipd) {
        ...
        foreach ($sss_ipd['transfers_by_account'] as $acc => $tr) {
            ...
            $batch_excel['1102050101.310']['total_debit'] += $tr['transfer_out'];
            $batch_excel['1102050101.310']['count'] += ($tr['full_transfer_cases'] ?? 0);
        }
    }
...
if ($action === 'get_account_audit_detail') {
    ...
    if ($cr_opd) {
        if ($accountcode === '1102050101.216') {
            ...
            foreach ($cr_opd['transfers_by_account'] as $acc_k => $tr_k) {
                $excel_total += $tr_k['transfer_out'];
                $excel_count += $tr_k['full_transfer_cases'];
            }
    ...
    if ($cr_ipd) {
        if ($accountcode === '1102050101.217') {
            ...
            foreach ($cr_ipd['transfers_by_account'] as $acc_k => $tr_k) {
                $excel_total += $tr_k['transfer_out'];
                $excel_count += ($tr_k['full_transfer_cases'] ?? 0);
            }
    ...
    if ($sss_opd) {
        if ($accountcode === '1102050101.309') {
            ...
            foreach ($sss_opd['transfers_by_account'] as $acc_k => $tr_k) {
                $excel_total += $tr_k['transfer_out'];
                $excel_count += $tr_k['full_transfer_cases'];
            }
    ...
    if ($sss_ipd) {
        if ($accountcode === '1102050101.310') {
            ...
            foreach ($sss_ipd['transfers_by_account'] as $acc_k => $tr_k) {
                $excel_total += $tr_k['transfer_out'];
                $excel_count += ($tr_k['full_transfer_cases'] ?? 0);
            }
