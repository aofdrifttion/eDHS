...
if (isset($_POST['month'])) {
    ...
    $monthlasts = $_POST['monthlasts']; // รับค่าจาก AJAX 
    ...
    // Safeguard: ถ้าไม่ได้เปิดระบบ CR IPD ให้ยุบผังลูก 1102050101.217 รวมเข้ากับ 1102050101.202 ทันที ป้องกันหลุดแสดงผล
    ...
    SUM(main_query.total_debitlasts) AS total_debitlasts,
...
FROM (
    SELECT 
        ...
        SUM(debitlasts) AS total_debitlasts,
    ...
    FROM (
        SELECT 
            ...
            SUM(IF(monthtxt = '$monthlasts', debit, 0)) AS debitlasts,
...
GROUP BY 
    ...
    if (mysqli_num_rows($result) > 0) {
     ...
     $day90=0; $adebitlast = 0;$adebitlasts = 0;$acchnlast = 0;$adebit = 0; $acolumn6 =0; $acolumn7 =0; $acolumn8 =0; $acolumn9 =0; $acolumn10 =0; $acolumn11 =0; $acolumn12 =0;$acolumn10last =0;
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        $adebitlasts = $adebitlasts+$row["total_debitlasts"]+$row["total_debit"]+$row["total_debitlast"]; // 90 วันย้อนหลัง
        ...
        if($row["column10last"] === '0.00'){
            $day90 = 0;
        }else{
            $day90 = $row["total_debitlasts"]+$row["total_debit"]+$row["total_debitlast"];
        }
          ...
          <td style="text-align: right;color: #0004a1;" class="numericcolor column3"><a href="debtor_excel.php?acc=' . $row["accountcode"] . '&month='.$monthlast.'" >' . $row["column10last"] . '</a></td>
          ...
          <td style="text-align: right;background: #fff3d7;" class="numericcolor column6 locked-cell" contenteditable="false">' . number_format(!empty($row["column6"]) ? $row["column6"] : 0, 2) . '</td>
          <td style="text-align: right;" class="numericcolor column7">' . $row["column7"] . '</td>
          <td style="text-align: right;background: #f2ffd7;" class="numericcolor column8 locked-cell" contenteditable="false">' . number_format(!empty($row["column8"]) ? $row["column8"] : 0, 2) . '</td>
          <td style="text-align: right;background: #f2ffd7;" class="numericcolor column9 locked-cell" contenteditable="false">' . number_format(!empty($row["column9"]) ? $row["column9"] : 0, 2) . '</td>

          <td style="text-align: right;background: #d7ffda;" class="numericcolor column10 locked-cell" contenteditable="false">' . number_format(!empty($row["column10"]) ? $row["column10"] : 0, 2) . '</td>
          ...
          <td style="text-align: right;" class="numericcolor column13 locked-cell" contenteditable="false">' . $row["column13"] . '</td>
          <td style="text-align: right;" class="numericcolor column14 locked-cell" contenteditable="false">' . $row["column14"] . '</td>
          <td style="text-align: right;" class="numericcolor column15 locked-cell" contenteditable="false">' . $row["column15"] . '</td>
          <td style="text-align: right;" class="numericcolor column16 locked-cell" contenteditable="false">' . $row["column16"] . '</td>
    ...
    <tr>
    <td colspan="14" style="bold;border: none;height: 65px; "></td>
    </tr>
    ...
    <tr>
    <td colspan="14" style="bold;border: none;height: 55px; "></td>
    </tr>
