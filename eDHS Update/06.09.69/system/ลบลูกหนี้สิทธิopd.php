...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'delete_opd') {
    ...
    $sql_check = "SELECT COUNT(*) as count FROM imr_tb_debtor_rights_opd WHERE monthtxt = ? AND ((bill IS NOT NULL AND TRIM(bill) != '') OR (follow_money IS NOT NULL AND CAST(follow_money AS DECIMAL(15,2)) > 0))";
    ...
    if ($chk_tbl_cr && $chk_tbl_cr->num_rows > 0) {
        $stmt_check_cr = $conn->prepare("SELECT COUNT(*) as count FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND monthtxt = ? AND ((bill IS NOT NULL AND TRIM(bill) != '') OR (compensated IS NOT NULL AND compensated > 0))");
        if ($stmt_check_cr) {
    ...
    if ($chk_tbl_sss && $chk_tbl_sss->num_rows > 0) {
        $stmt_check_sss = $conn->prepare("SELECT COUNT(*) as count FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND monthtxt = ? AND ((bill IS NOT NULL AND TRIM(bill) != '') OR (compensated IS NOT NULL AND compensated > 0))");
        if ($stmt_check_sss) {
    ...
    if ($billed_main > 0 || $billed_cr > 0 || $billed_sss > 0) {
        ...
        echo json_encode([
            'status' => 'error', 
            'message' => 'ไม่สามารถลบได้ เนื่องจากมีรายการที่ออกใบเสร็จหรือได้รับเงินชดเชยแล้วในเดือนนี้ (' . implode(', ', $block_reasons) . ')'
        ]);
        exit;
    }
    ...
    try {
        // 1. ลบจากตารางหลัก imr_tb_debtor_rights_opd
        $stmt_main = $conn->prepare("DELETE FROM imr_tb_debtor_rights_opd WHERE monthtxt = ?");
        ...
        if ($chk_tbl_cr && $chk_tbl_cr->num_rows > 0) {
            $stmt_del_cr = $conn->prepare("DELETE FROM imr_tb_debtor_cr_breakdown WHERE visit_type = 'OPD' AND monthtxt = ?");
            if ($stmt_del_cr) {
        ...
        if ($chk_tbl_sss && $chk_tbl_sss->num_rows > 0) {
            $stmt_del_sss = $conn->prepare("DELETE FROM imr_tb_debtor_sss_breakdown WHERE visit_type = 'OPD' AND monthtxt = ?");
            if ($stmt_del_sss) {
        ...
        if (function_exists('system_log')) {
            system_log($conn, 'ระบบลูกหนี้ (Delete)', 'DELETE', "ลบข้อมูลลูกหนี้สิทธิ OPD เดือน $monthtxt พร้อมล้างข้อมูลกลุ่มย่อย CR และ ประกันสังคม เรียบร้อยแล้ว");
        }
