        ...
        if ($stmt_p) {
            ...
            foreach ($rows_raw as $r) {
                ...
                $patients[] = [
                    ...
                    'paid_money' => floatval($r['paid_money']) + floatval($r['rcpt_money']),
        ...
        if ($is_last_batch_opd) {
            ...
            if (file_exists($config_file)) {
                ...
                if (isset($config_data['debtor_setting']) && ($config_data['debtor_setting']['enabled'] ?? '0') === '1') {
                    ...
                    $is_m_effective = (empty($start_m) || strtoupper($start_m) === 'ALL' || (function_exists('compare_monthtxt_be') && compare_monthtxt_be($monthtxt, $start_m) >= 0));
 ...
 * ====================================================================================
 * ACTION: delete_batch
 * ลบข้อมูลลูกหนี้สิทธิตามงวดเดือน/สิทธิ พร้อม Triple Financial Safeguard ตาม Rule 87
 * ====================================================================================
 */
if ($action === 'delete_batch') {
    $monthtxt = isset($_REQUEST['monthtxt']) ? trim($_REQUEST['monthtxt']) : '';
    $patient_type = isset($_REQUEST['patient_type']) ? strtolower(trim($_REQUEST['patient_type'])) : 'both';
    $target_acc = isset($_REQUEST['accountcode']) ? trim($_REQUEST['accountcode']) : '';
    
    if (empty($monthtxt)) {
        send_response('error', 'กรุณาระบุงวดเดือนที่ต้องการลบ (เช่น 8-2026)');
    }
    
    // -------------------------------------------------------------
    // ตรวจสอบความปลอดภัยทางการเงิน (Triple Financial Safeguard)
    // -------------------------------------------------------------
    $escaped_month = mysqli_real_escape_string($conn, $monthtxt);
    $acc_cond = !empty($target_acc) ? " AND accountcode = '" . mysqli_real_escape_string($conn, $target_acc) . "' " : "";
    ...
    // 1. ตรวจสอบการยืนยันตั้งลูกหนี้ในระบบทะเบียนคุมลูกหนี้ (imr_tb_debtor_result.column10 > 0)
    $q_deb_chk = mysqli_query($conn, "SELECT SUM(column10) as total_c10 FROM imr_tb_debtor_result WHERE month = '$escaped_month' $acc_cond_result");
    $total_c10_confirmed = floatval(mysqli_fetch_assoc($q_deb_chk)['total_c10'] ?? 0);
    if ($total_c10_confirmed > 0) {
        send_response('error', "บล็อกการลบข้อมูลงวด $monthtxt เนื่องจากมีการยืนยันตั้งลูกหนี้ในระบบทะเบียนคุมลูกหนี้ (imr_tb_debtor_result) แล้ว จำนวน " . number_format($total_c10_confirmed, 2) . " บาท เพื่อความปลอดภัยทางบัญชี ห้ามลบข้อมูลเด็ดขาด");
    }

    // 2. ตรวจสอบประวัติการรับชำระเงินใน imr_tb_payment_history
    $q_ph = mysqli_query($conn, "SELECT COUNT(*) as c FROM imr_tb_payment_history ph 
        WHERE ph.ref_vn_an IN (SELECT vn FROM imr_tb_debtor_rights_opd WHERE monthtxt = '$escaped_month' $acc_cond)
           OR ph.ref_vn_an IN (SELECT an FROM imr_tb_debtor_rights_ipd WHERE monthtxt = '$escaped_month' $acc_cond)");
    $has_pay_history = intval(mysqli_fetch_assoc($q_ph)['c'] ?? 0);
    if ($has_pay_history > 0) {
        send_response('error', "บล็อกการลบข้อมูลงวด $monthtxt เนื่องจากตรวจพบประวัติการชำระเงิน/ออกใบเสร็จในระบบ (imr_tb_payment_history) จำนวน $has_pay_history รายการ เพื่อความปลอดภัยทางบัญชี ห้ามลบข้อมูลเด็ดขาด");
    }

    // 3. ตรวจสอบตาราง OPD (ครอบคลุมทั้ง bill, mobile และ follow_money)
    $has_opd_bill = 0;
    $has_ipd_bill = 0;
    $has_cr_bill = 0;
    $has_sss_bill = 0;
    ...
    $billed_cond_opd = "((bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-') OR (mobile IS NOT NULL AND TRIM(mobile) != '' AND TRIM(mobile) != '-') OR (follow_money IS NOT NULL AND CAST(follow_money AS DECIMAL(15,2)) > 0))";
    if ($patient_type === 'opd' || $patient_type === 'both') {
        $q1 = mysqli_query($conn, "SELECT COUNT(*) as c FROM imr_tb_debtor_rights_opd WHERE monthtxt = '$escaped_month' $acc_cond AND $billed_cond_opd");
        $has_opd_bill = intval(mysqli_fetch_assoc($q1)['c'] ?? 0);
    }

    // 4. ตรวจสอบตาราง IPD (ครอบคลุมทั้ง bill, mobile และ follow_money)
    $billed_cond_ipd = "((bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-') OR (mobile IS NOT NULL AND TRIM(mobile) != '' AND TRIM(mobile) != '-') OR (follow_money IS NOT NULL AND CAST(follow_money AS DECIMAL(15,2)) > 0))";
    if ($patient_type === 'ipd' || $patient_type === 'both') {
        $q2 = mysqli_query($conn, "SELECT COUNT(*) as c FROM imr_tb_debtor_rights_ipd WHERE monthtxt = '$escaped_month' $acc_cond AND $billed_cond_ipd");
        $has_ipd_bill = intval(mysqli_fetch_assoc($q2)['c'] ?? 0);
    }

    // 5. ตรวจสอบตาราง CR Breakdown
    $q3 = mysqli_query($conn, "SELECT COUNT(*) as c FROM imr_tb_debtor_cr_breakdown WHERE monthtxt = '$escaped_month' AND ((bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-') OR (compensated IS NOT NULL AND CAST(compensated AS DECIMAL(15,2)) > 0))");
    $has_cr_bill = intval(mysqli_fetch_assoc($q3)['c'] ?? 0);

    // 6. ตรวจสอบตาราง SSS Breakdown
    $q4 = mysqli_query($conn, "SELECT COUNT(*) as c FROM imr_tb_debtor_sss_breakdown WHERE monthtxt = '$escaped_month' AND ((bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-') OR (compensated IS NOT NULL AND CAST(compensated AS DECIMAL(15,2)) > 0))");
    $has_sss_bill = intval(mysqli_fetch_assoc($q4)['c'] ?? 0);

    $total_blocked = $has_opd_bill + $has_ipd_bill + $has_cr_bill + $has_sss_bill;
    if ($total_blocked > 0) {
        send_response('error', "บล็อกการลบข้อมูลงวด $monthtxt เนื่องจากมีรายการที่ออกใบเสร็จรับเงินหรือตัดหนี้แล้ว จำนวน $total_blocked รายการ (OPD: $has_opd_bill, IPD: $has_ipd_bill, CR: $has_cr_bill, ประกันสังคม: $has_sss_bill) เพื่อความปลอดภัยทางบัญชี");
    }

    // ดำเนินการลบแบบ Cascade ภายใต้ Transaction
    ...
    try {
        ...
        if ($patient_type === 'opd' || $patient_type === 'both') {
            // ลบ Breakdown ที่สัมพันธ์กัน
            mysqli_query($conn, "DELETE cr FROM imr_tb_debtor_cr_breakdown cr JOIN imr_tb_debtor_rights_opd o ON o.vn = cr.vn AND cr.visit_type = 'OPD' WHERE o.monthtxt = '$escaped_month' $acc_cond_opd");
            mysqli_query($conn, "DELETE sss FROM imr_tb_debtor_sss_breakdown sss JOIN imr_tb_debtor_rights_opd o ON o.vn = sss.vn AND sss.visit_type = 'OPD' WHERE o.monthtxt = '$escaped_month' $acc_cond_opd");
            // ลบจากตารางหลัก OPD
            mysqli_query($conn, "DELETE FROM imr_tb_debtor_rights_opd WHERE monthtxt = '$escaped_month' $acc_cond");
            $deleted_opd = mysqli_affected_rows($conn);
        }
        
        if ($patient_type === 'ipd' || $patient_type === 'both') {
            mysqli_query($conn, "DELETE cr FROM imr_tb_debtor_cr_breakdown cr JOIN imr_tb_debtor_rights_ipd i ON i.an = cr.vn AND cr.visit_type = 'IPD' WHERE i.monthtxt = '$escaped_month' $acc_cond_ipd");
            mysqli_query($conn, "DELETE sss FROM imr_tb_debtor_sss_breakdown sss JOIN imr_tb_debtor_rights_ipd i ON i.an = sss.vn AND sss.visit_type = 'IPD' WHERE i.monthtxt = '$escaped_month' $acc_cond_ipd");
            mysqli_query($conn, "DELETE FROM imr_tb_debtor_rights_ipd WHERE monthtxt = '$escaped_month' $acc_cond");
        ...
        if (function_exists('system_log')) {
            system_log($conn, 'ลบลูกหนี้สิทธิ', 'DELETE_SAFE', "ลบข้อมูลลูกหนี้งวด $monthtxt สำเร็จ (OPD: $deleted_opd, IPD: $deleted_ipd)");
        }
        
        send_response('success', "ลบข้อมูลลูกหนี้งวด $monthtxt สำเร็จ", [
            'monthtxt' => $monthtxt,
