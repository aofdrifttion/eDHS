...
if (isset($_POST['month']) && isset($_POST['monthlast'])) {
    ...
    function getDataByMonth($conn, $sql_template, $target_month, $last_month) {
        ...
        $cr_opd = (is_cr_effective_for_month($cr_config, $target_month) && $auto_split_opd)
            ? get_cr_splitting_summary_for_month($conn, $conn2 ?? null, $target_month, 'OPD', $cr_config, $my_hospcode) : null;
        $cr_ipd = (is_cr_effective_for_month($cr_config, $target_month) && $auto_split_ipd)
            ? get_cr_splitting_summary_for_month($conn, $conn2 ?? null, $target_month, 'IPD', $cr_config, $my_hospcode) : null;
        ...
        $sss_opd = (is_sss_effective_for_month($sss_config, $target_month) && $auto_split_sss_opd)
            ? get_sss_splitting_summary_for_month($conn, $conn2 ?? null, $target_month, 'OPD', $sss_config, $my_hospcode) : null;
        $sss_ipd = (is_sss_effective_for_month($sss_config, $target_month) && $auto_split_sss_ipd)
            ? get_sss_splitting_summary_for_month($conn, $conn2 ?? null, $target_month, 'IPD', $sss_config, $my_hospcode) : null;
