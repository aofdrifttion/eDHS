    ...
    function ensure_cr_tables($conn) {
        ...
        $sql_breakdown = "
            CREATE TABLE IF NOT EXISTS `imr_tb_debtor_cr_breakdown` (
                ...
                UNIQUE KEY `uniq_visit_vn` (`visit_type`, `vn`),
        ...
        // Auto-migrate index from uniq_visit_vn_cr to uniq_visit_vn (visit_type, vn)
        $q_chk_old_idx = mysqli_query($conn, "SHOW INDEX FROM `imr_tb_debtor_cr_breakdown` WHERE Key_name = 'uniq_visit_vn_cr'");
        $q_chk_new_idx = mysqli_query($conn, "SHOW INDEX FROM `imr_tb_debtor_cr_breakdown` WHERE Key_name = 'uniq_visit_vn'");
        if (($q_chk_old_idx && mysqli_num_rows($q_chk_old_idx) > 0) || ($q_chk_new_idx && mysqli_num_rows($q_chk_new_idx) === 0)) {
            // 1. Deduplicate existing records
            $q_dup = mysqli_query($conn, "SELECT visit_type, vn, COUNT(*) as cnt FROM `imr_tb_debtor_cr_breakdown` GROUP BY visit_type, vn HAVING cnt > 1");
            if ($q_dup && mysqli_num_rows($q_dup) > 0) {
                if ($q_chk_old_idx && mysqli_num_rows($q_chk_old_idx) > 0) {
                    @mysqli_query($conn, "ALTER TABLE `imr_tb_debtor_cr_breakdown` DROP INDEX `uniq_visit_vn_cr`");
                }
                while ($d = mysqli_fetch_assoc($q_dup)) {
                    $vt = $d['visit_type'];
                    $v = mysqli_real_escape_string($conn, $d['vn']);
                    $q_rows = mysqli_query($conn, "SELECT id, cr_type, bill, compensated FROM `imr_tb_debtor_cr_breakdown` WHERE visit_type = '$vt' AND vn = '$v' ORDER BY CASE WHEN bill IS NOT NULL AND TRIM(bill) != '' AND TRIM(bill) != '-' THEN 1 ELSE 2 END ASC, CASE WHEN compensated IS NOT NULL AND compensated > 0 THEN 1 ELSE 2 END ASC, id DESC");
                    $ids = [];
                    $best_id = null;
                    ...
                    if (!empty($ids)) {
                        $del_str = implode(',', $ids);
                        @mysqli_query($conn, "DELETE FROM `imr_tb_debtor_cr_breakdown` WHERE id IN ($del_str)");
                    }
                }
            } elseif ($q_chk_old_idx && mysqli_num_rows($q_chk_old_idx) > 0) {
                @mysqli_query($conn, "ALTER TABLE `imr_tb_debtor_cr_breakdown` DROP INDEX `uniq_visit_vn_cr`");
            }
            // 2. Add new unique index
            $q_chk_new = mysqli_query($conn, "SHOW INDEX FROM `imr_tb_debtor_cr_breakdown` WHERE Key_name = 'uniq_visit_vn'");
            if ($q_chk_new && mysqli_num_rows($q_chk_new) === 0) {
                @mysqli_query($conn, "ALTER TABLE `imr_tb_debtor_cr_breakdown` ADD UNIQUE KEY `uniq_visit_vn` (`visit_type`, `vn`)");
            }
        }
