            ...
            <tbody>
                  ...
                  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['select'])) {
                      ...
                      if (!empty($safe_selected)) {
                          $q_cr_bd = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_cr_breakdown WHERE visit_type = '$typecc' AND vn IN ($safe_selected)");
                          if ($q_cr_bd) {
                      ...
                      $sss_bd_map = [];
                      if (!empty($safe_selected)) {
                          $q_sss_bd = mysqli_query($conn, "SELECT vn, item_amount, general_remain_amount, compensated, bill, billdate, settle_status FROM imr_tb_debtor_sss_breakdown WHERE visit_type = '$typecc' AND vn IN ($safe_selected)");
                          if ($q_sss_bd) {
