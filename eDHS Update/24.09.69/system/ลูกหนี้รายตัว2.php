                                  ...
                                  if ($is_cr_child_account && isset($cr_visits_map[$ref_id])) {
                                   ...
                                   if ($total_payments > 0) {
                                       ...
                                       if ($is_cr_child_account) {
                                           ...
                                           $child_cr_bill = trim($cr_bd_map[$ref_id]['bill'] ?? ($cr_visits_map[$ref_id]['bill'] ?? ''));
                                           $is_mother_debt_settled = ($mother_comp > 0 && ($income - $mother_comp) <= 0.01);
                                           $mb = $extract_mobile_bill($row["mobile"]);
                                           $current_bill = !empty($row["bill"]) ? trim($row["bill"]) : trim($mb['bill'] ?? '');

                                           if (!empty($child_cr_bill) && $current_bill === $child_cr_bill && !$is_mother_debt_settled) {
                                               $bill = '';
                                               $billdate = '';
                                           } else {
                                               if (!empty($row["bill"])) {
                                           ...
                                           // ถ้าเลขใบเสร็จในตารางหลักตรงกับใบเสร็จของผังลูก และหนี้ผังแม่ยังไม่จบ ให้เคลียร์เพื่อรอตัดหนี้ผังแม่
                                           $child_sss_bill = trim($sss_bd_map[$ref_id]['bill'] ?? ($sss_visits_map[$ref_id]['bill'] ?? ''));
                                           $is_mother_debt_settled = ($mother_comp > 0 && ($income - $mother_comp) <= 0.01);
                                           $mb = $extract_mobile_bill($row["mobile"]);
                                           $current_bill = !empty($row["bill"]) ? trim($row["bill"]) : trim($mb['bill'] ?? '');

                                           if (!empty($child_sss_bill) && $current_bill === $child_sss_bill && !$is_mother_debt_settled) {
                                               $bill = '';
                                               $billdate = '';
                                           } else {
                                               if (!empty($row["bill"])) {
