...
if (!empty($_FILES['excel_file']['name'][0])) {
    ...
    // 🔥 สเต็ป 2: Refined Search (วนลูปเก็บตกเคส IPD หรือเคสที่วันที่ไม่ตรงเป๊ะ)
    $fetch_sql = "SELECT id, hn, dttran, stm_doc FROM imr_tb_seamless_dckd_ofc WHERE LENGTH(vn) < 12";
    $res = $conn->query($fetch_sql);

    if ($res && $res->num_rows > 0) {
        // เตรียม Statement ไว้ล่วงหน้าลดภาระ DB
