...
/*              $(".loading")
      ...
      function viewopdlist(e)
        ...
        function checkAllbilladd(status) {
            ...
            const payment = $('input[name="payment"]').val();
      ...
      function updatePaymentPreview() {
          ...
          // ===== แสดงผล realtime =====
          let showFront = front.padStart(4, '0'); // เติมขั้นต่ำ 3 หลัก
          let showBack  = back.padStart(4, '0');  // หลังต้อง 3 หลัก
      ...
      $(document).on('blur', '.mask-bill', function() {
          ...
          // ยังไม่มีหลัง → ไม่ต้อง format
          if (back.length === 0) return;

          // ===== จัดหลัง =====
          back = back.padStart(4, '0');

          // ===== จัดหน้า =====
          if (front.length <= 4) {
              // เติมให้ครบ 3 หลัก
              front = front.padStart(4, '0');
          } else {
              // กรณี 4 หลัก
              if (front.startsWith('0')) {
                  // ถ้ามี 0 นำ → ใช้ 3 ตัวท้าย
                  front = front.slice(-4).padStart(4, '0');
              } else {
                  // ใช้ 4 หลักตามเดิม
                  front = front.substring(0, 4);
              }
