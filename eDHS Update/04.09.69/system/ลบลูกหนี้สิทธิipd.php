...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'delete_ipd') {
    ...
    $sql_check = "SELECT COUNT(*) as count FROM imr_tb_debtor_rights_ipd WHERE monthtxt = ? AND bill IS NOT NULL AND bill != ''";
    ...
    $sql = "DELETE FROM imr_tb_debtor_rights_ipd WHERE monthtxt = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $monthtxt);

    if ($stmt->execute()) {
        if (function_exists('system_log')) {
            system_log($conn, 'ระบบลูกหนี้ (Delete)', 'DELETE', "ลบข้อมูลลูกหนี้สิทธิ IPD เดือน $monthtxt เรียบร้อยแล้ว");
        }
