...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'delete_stm') {
    ...
    try {
        // 3. ดึงเลข vn ทั้งหมดภายใต้ rep นี้ออกมาเพื่อไปอัปเดตตารางอื่น
        $stmt_select = $conn->prepare("SELECT vn FROM imr_tb_check_invoice WHERE rep = ?");
        $stmt_select->bind_param("s", $deletestm);
        $stmt_select->execute();
        ...
        $stmt_del = $conn->prepare("DELETE FROM imr_tb_check_invoice WHERE rep = ?");
        $stmt_del->bind_param("s", $deletestm);
        $stmt_del->execute();

        // ถ้ามาถึงตรงนี้โดยไม่มี Error ให้ยืนยันการเปลี่ยนแปลงทั้งหมด
        $conn->commit();
        if (function_exists('system_log')) {
            system_log($conn, 'นำเข้า STM & ตัดลูกหนี้ตาม STM', 'DELETE', "ลบข้อมูล STM (Rep: $deletestm)");
        }
        echo json_encode(['status' => 'success', 'message' => 'ลบข้อมูล STM เรียบร้อยแล้ว']);
