...
try {
    ...
    // อัปเดตตั้งค่าภาระหนี้อัตโนมัติ (ถ้าเปิดระบบไว้)
    ...
    if (file_exists($config_file)) {
        ...
        if (isset($config_data['debtor_setting']) && $config_data['debtor_setting']['enabled'] === '1') {
            $amount = floatval($config_data['debtor_setting']['amount']);
            $conn->query("UPDATE imr_tb_debtor_rights_ipd SET original_debit = debit WHERE accountcode IN ('1102050101.203') AND original_debit IS NULL");
            $conn->query("UPDATE imr_tb_debtor_rights_ipd SET debit = $amount WHERE accountcode IN ('1102050101.203')");
        }
    }
    ...
    $log_detail = "นำเข้าข้อมูล IPD เดือน $monthtxt สำเร็จ (เพิ่ม: $insert_count, อัปเดต: $update_count, ข้าม: $skipped_count จาก " . count($selected_rights) . " สิทธิที่เลือก)";
