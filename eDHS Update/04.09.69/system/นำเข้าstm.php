                  ...
                  <div class="stm-section-card">
                    <form id="upload-formSTM" enctype="multipart/form-data" class="stm-box-general">
                      ...
                      <div class="row g-3 align-items-end">
                        <div class="col-md-7">
                          <label class="stm-field-label"><i class='bx bx-bookmark-alt text-success'></i> ตรวจสอบเลขที่หนังสือที่นำเข้าตัดลูกหนี้</label>
                          <select class="form-select stm-select" id="deletestm" name="deletestm">
                            <option value="00" selected disabled>ตรวจสอบเลขที่หนังสือที่นำเข้าตัดลูกหนี้</option>
                            <?php 
                            $sqlzm = "SELECT rep FROM imr_tb_check_invoice GROUP By rep order by rep DESC";
                            $resultzm = mysqli_query($conn, $sqlzm);
                            if (mysqli_num_rows($resultzm) > 0) {
                              while ($rowzm = mysqli_fetch_assoc($resultzm)) {
                                $gmm = $rowzm['rep'];
                                echo'<option value="'.$gmm.'">STM เลขที่หนังสือที่ '.$gmm.'</option>';
                              }
                            }
                            ?>
                          </select>
                        ...
                        <div class="col-md-5">
                          <div class="d-flex align-items-center justify-content-end gap-2">
                            <button id="submit_stm" disabled type="submit" class="btn btn-stm-submit" data-bs-placement="bottom" data-bs-toggle="tooltip" title="นำเข้าข้อมูล STM">
                              <i class="bx bx-cloud-upload"></i> นำเข้าข้อมูล STM
                            </button>
...
<script src="https://code.iconify.design/iconify-icon/1.0.7/iconify-icon.min.js"></script>
    ...
    <script>
      ...
      $('#submit_stm').click(function () {
          // ย้าย submit handler ออกจาก click เพื่อป้องกันซ้อนซ้อน
...
//DELETE stm
      ...
      $('#confirm_deletes').on('click', function () {
          ...
          if (deletestm === '00') {
            $("#upload-formSTM").trigger("reset");
            $('#passwordModals').modal('hide');
            Swal.fire('แจ้งเตือน', 'กรุณาเลือกเลข REP ก่อนลบข้อมูล', 'warning');
            return;
          }else{
