 ...
 * 
 * OPD: ตัด Instrument จาก .301, .303, .307, .308 เข้า .309
 * IPD: ตัด Instrument จาก .302, .304 เข้า .310
 * 
    ...
    function ensure_sss_tables($conn) {
        ...
        $sql_breakdown = "
            CREATE TABLE IF NOT EXISTS `imr_tb_debtor_sss_breakdown` (
                ...
                `parent_accountcode` VARCHAR(50) NOT NULL COMMENT 'รหัสผังบัญชีหลักเดิม เช่น 1102050101.301, .303, .307, .308, .302, .304',
    ...
    function get_default_sss_config() {
        return [
            ...
            "parent_accounts" => [
                ...
                "1102050101.308" => ["enabled" => true, "type" => "OPD", "name" => "ลูกหนี้ค่ารักษาประกันสังคม 72 ชั่วโมงแรก"],
                "1102050101.309" => ["enabled" => true, "type" => "OPD", "name" => "ลูกหนี้ค่ารักษาประกันสังคม - ค่าใช้จ่ายสูง/อุบัติเหตุ/ฉุกเฉิน OP"],
                // ผัง IPD ประกันสังคม (3 ผัง)
...
if (!function_exists('detect_sss_types_for_visit_list')) {
    ...
    function detect_sss_types_for_visit_list($conn2, $vn_list, $visit_type = 'OPD', $config = null, $my_hospcode = '', $monthtxt = null) {
        ...
        $is_inst_enabled = is_sss_subgroup_enabled($config, 'SSS-Instrument', $visit_type, $monthtxt);
        $inst_cfg = isset($subgroups_cfg['SSS-Instrument']) ? $subgroups_cfg['SSS-Instrument'] : [];
        $inst_adps = !empty($inst_cfg['custom_adp_codes']) ? array_map('strval', $inst_cfg['custom_adp_codes']) : ["7004", "7005", "8612", "8813", "8814"];
        $inst_icodes = !empty($inst_cfg['custom_icodes']) ? array_map('strval', $inst_cfg['custom_icodes']) : [];
        ...
        if ($q_items && mysqli_num_rows($q_items) > 0) {
            while ($row = mysqli_fetch_assoc($q_items)) {
                ...
                if ($is_inst_enabled) {
                    $is_match = false;
                    $adp_code = (string)($row['nondrug_adp_code'] ?: $row['drug_adp_code']);
                    $adp_type = (string)($row['nondrug_adp_type'] ?: $row['drug_adp_type']);
                    $icode = (string)$row['icode'];

                    if ($auto_adp2 && ($adp_type === '2')) {
                        $is_match = true;
                    }
                    if (!$is_match && !empty($inst_adps)) {
                        if (in_array($adp_code, $inst_adps) || preg_match('/^700[4-5]|^8612|^8813|^8814/', $adp_code)) {
                            $is_match = true;
                        }
                    }
                    if (!$is_match && !empty($inst_icodes) && in_array($icode, $inst_icodes)) {
                        $is_match = true;
                    }
...
if (!function_exists('get_sss_splitting_summary_for_month')) {
    ...
    function get_sss_splitting_summary_for_month($conn, $conn2, $monthtxt, $visit_type = 'OPD', $config = null, $my_hospcode = '', $vn_filter = null) {
        ...
        if ($visit_type === 'OPD') {
            $raw_parents = ['1102050101.301', '1102050101.303', '1102050101.307', '1102050101.308'];
            $parent_accounts = [];
            ...
            $raw_parents = ['1102050101.302', '1102050101.304'];
            $parent_accounts = [];
            foreach ($raw_parents as $pa) {
                if (is_sss_parent_account_enabled($config, $pa)) {
                    $parent_accounts[] = $pa;
        ...
        if (!empty($all_vns)) {
            ...
            $stm_join = ($visit_type === 'IPD') ? "stm.an = o.an" : "stm.vn = o.vn";
