                    ...
                    <form id="upload-formOPD" enctype="multipart/form-data" class="import-box-opd">
                      ...
                      <div class="mb-3">
                        ...
                        <div class="form-check form-switch mt-2">
                          <input class="form-check-input" type="checkbox" id="skip_zero_debit_opd" name="skip_zero_debit" value="1" checked>
                          <label class="form-check-label fw-semibold text-dark" for="skip_zero_debit_opd" style="font-size: 13.5px; cursor: pointer;">
                            <i class='bx bx-filter-alt text-warning me-1'></i> ข้ามรายการที่ภาระหนี้เป็น 0 (ไม่นำเข้าเคสที่ debit = 0)
                          </label>
                          <div class="form-text text-muted ms-4" style="font-size: 11.5px;">
                            *แนะนำ: ลดความซ้ำซ้อนและป้องกันรายการ 0 บาทในทะเบียนคุม (ยกเว้นผังที่ต้องคำนวณชดเชยตามเกณฑ์ eDHS)
                          </div>
                        </div>
                      </div>
                  ...
                  <div class="import-section-card">
                    <form id="upload-formIPD" enctype="multipart/form-data" class="import-box-ipd">
                      ...
                      <div class="mb-3">
                        ...
                        <input class="form-control import-file-control" name="excel_file" type="file" required />
                        <div class="form-check form-switch mt-2">
                          <input class="form-check-input" type="checkbox" id="skip_zero_debit_ipd" name="skip_zero_debit" value="1" checked>
                          <label class="form-check-label fw-semibold text-dark" for="skip_zero_debit_ipd" style="font-size: 13.5px; cursor: pointer;">
                            <i class='bx bx-filter-alt text-primary me-1'></i> ข้ามรายการที่ภาระหนี้เป็น 0 (ไม่นำเข้าเคสที่ debit = 0)
                          </label>
                          <div class="form-text text-muted ms-4" style="font-size: 11.5px;">
                            *แนะนำ: ลดความซ้ำซ้อนและป้องกันรายการ 0 บาทในทะเบียนคุม (ยกเว้นผังที่ต้องคำนวณชดเชยตามเกณฑ์ eDHS)
                          </div>
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <!-- Zero Debit Option Banner -->
        <div class="alert alert-light border d-flex flex-wrap align-items-center justify-content-between p-2 mb-3 rounded gap-2 shadow-sm">
          <div class="form-check form-switch mb-0">
            <input class="form-check-input" type="checkbox" id="modalSkipZeroDebit" checked>
            <label class="form-check-label fw-bold text-dark" for="modalSkipZeroDebit" style="font-size: 13.5px; cursor: pointer;">
              <i class='bx bx-filter-alt text-primary me-1'></i> ข้ามรายการที่ภาระหนี้เป็น 0 (debit = 0)
            </label>
            <span class="badge bg-label-warning ms-2" id="badgeZeroDebitCount" style="display:none; font-size: 12.5px;"></span>
          </div>
          <small class="text-muted fst-italic">*ยกเว้นผังที่ eDHS ต้องคำนวณยอดชดเชยต่อ (เช่น 203, CR, SSS)</small>
        </div>
...
<script src="https://code.iconify.design/iconify-icon/1.0.7/iconify-icon.min.js"></script>
    ...
    <script>
      ...
      function showRightsPreviewModal(res, type) {
          ...
          // ซิงค์ค่า Checkbox จากหน้าฟอร์มมายัง Modal และแสดงจำนวนรายการหนี้เป็น 0
          var initialSkip = (type === 'OPD') ? $('#skip_zero_debit_opd').is(':checked') : $('#skip_zero_debit_ipd').is(':checked');
          $('#modalSkipZeroDebit').prop('checked', initialSkip);
          if (res.zero_debit_count && res.zero_debit_count > 0) {
              $('#badgeZeroDebitCount').text(`พบภาระหนี้ 0 บาท: ${res.zero_debit_count.toLocaleString()} รายการ`).show();
          } else {
              $('#badgeZeroDebitCount').hide();
          }
      ...
      $(document).on('click', '#btnConfirmImportRights', function() {
          ...
          var skipZeroDebit = $('#modalSkipZeroDebit').is(':checked') ? 1 : 0;

          $('#modalSelectRights').modal('hide');

          if (currentImportContext.type === 'OPD') {
              executeImportOPD(currentImportContext, selectedRights, skipZeroDebit);
          } else {
              executeImportIPD(currentImportContext, selectedRights, skipZeroDebit);
          }
      });
      ...
      function executeImportOPD(context, selectedRights, skipZeroDebit) {
          var taskId = Date.now().toString();
          if (typeof skipZeroDebit === 'undefined') {
              skipZeroDebit = $('#modalSkipZeroDebit').is(':checked') ? 1 : 0;
          }
                          ...
                          html: `
                              <div style="text-align:left; display:inline-block; font-size:16px;">
                                  ...
                                  <div style="display:flex; align-items:center; margin-bottom:8px;">
                                      ...
                                      <span style="width:140px;">ข้ามภาระหนี้ 0 บาท</span>
                                      <span>: ${res.skipped_zero} รายการ</span>
                                  </div>` : ''}
      ...
      function executeImportIPD(context, selectedRights, skipZeroDebit) {
          var taskId = Date.now().toString();
          if (typeof skipZeroDebit === 'undefined') {
              skipZeroDebit = $('#modalSkipZeroDebit').is(':checked') ? 1 : 0;
          }
