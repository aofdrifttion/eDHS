...
// 🔥 5. รับค่าตัวเลือกข้ามรายการภาระหนี้เป็น 0 (skip_zero_debit)
...
try {
    ...
    foreach ($data as $row) {
        ...
        if ($skip_zero_debit && $debit_idx !== false) {
            ...
            // เช่น 1102050101.203 (เหมาจ่าย 175 บาท), 1102050101.216 (CR OP), 1102050101.309 (SSS OP)
            $needs_calculation = (
                strpos($row_code, '1102050101.203') !== false ||
                strpos($row_code, '1102050101.216') !== false ||
                strpos($row_code, '1102050101.309') !== false
            );
    ...
    $log_detail = "นำเข้าข้อมูล OPD เดือน $monthtxt สำเร็จ (เพิ่ม: $insert_count, อัปเดต: $update_count, ข้ามสิทธิ: $skipped_count, ข้ามหนี้ 0: $skipped_zero_count จาก " . count($selected_rights) . " สิทธิที่เลือก)";
