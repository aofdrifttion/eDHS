...
$is_sss_parent_opd = in_array($acc, ['1102050101.301', '1102050101.303', '1102050101.307']);
$is_sss_parent_ipd = in_array($acc, ['1102050101.302', '1102050101.304', '1102050101.308']);
...
/**
 * ค้นหาและจัดรูปแบบเลขที่ใบเสร็จและวันที่ใบเสร็จจากทุกแหล่งข้อมูล (Multi-Source Resolution)
 * ตามลำดับความสำคัญ:
 * 1. ข้อมูลจากตารางย่อย breakdown (CR / SSS)
 * 2. คอลัมน์ bill และ billdate ในตารางสิทธิ์หลัก (imr_tb_debtor_rights_opd / ipd)
 * 3. ข้อความในคอลัมน์ mobile (กรณีเก็บ 'เลขที่-วันที่' เช่น 0049/0031-16/09/2569)
 * 4. ประวัติการชำระเงินใน imr_tb_payment_history (Rule 11)
 *
 * @param array $row ข้อมูลแถวลูกหนี้
 * @param mysqli $conn การเชื่อมต่อฐานข้อมูล
 * @param string|null $bd_bill เลขใบเสร็จจาก breakdown (ถ้ามี)
 * @param string|null $bd_billdate วันที่ใบเสร็จจาก breakdown (ถ้ามี)
 * @return array [$bill_no, $bill_date]
 */
function resolve_debtor_receipt($row, $conn, $bd_bill = null, $bd_billdate = null) {
    ...
    // 3. จาก mobile string (กรณี IPD หรือ OPD ที่เก็บ 0049/0031-16/09/2569)
    ...
    // 4. ค้นหาจาก imr_tb_payment_history (Rule 11) กรณีที่มีการชำระเงิน
    ...
    // จัดรูปแบบเลขที่ใบเสร็จ
    if (!empty($r_bill) && $r_bill !== '-' && $r_bill !== '0000/0000') {
        if (function_exists('format_debtor_bill_no')) {
            $r_bill = format_debtor_bill_no($r_bill);
        }
    } else {
        $r_bill = '-';
    }
...
SELECT * FROM (
    /* ==== OPD ==== */
    SELECT vn, vstdate, cid, ptname, mobile, billdate, bill, accountcode,
            (IFNULL(debit, 0) - 
                CASE
                    WHEN billdate IS NOT NULL AND billdate != '' 
                     AND billdate != '-' AND CHAR_LENGTH(billdate) >= 10
    ...
    /* ==== IPD ==== */
    SELECT an AS vn, dchdate AS vstdate, cid, ptname, mobile, billdate, bill, accountcode,
            (IFNULL(debit, 0) - 
                CASE
                    WHEN billdate IS NOT NULL AND billdate != '' 
                     AND billdate != '-' AND CHAR_LENGTH(billdate) >= 10
...
if ($is_parent_acc) {
    ...
    if (!empty($missing_parent_ids)) {
        ...
        foreach ($id_chunks as $chunk) {
            ...
            $sql_parent = "
                SELECT $pk_col AS vn, $date_select, cid, ptname, mobile, billdate, bill, accountcode, pttypename
                FROM $pt_table
            ...
            if ($res_parent) {
                while ($r_p = $res_parent->fetch_assoc()) {
                    $pid = $r_p['vn'];
                    $r_p['type'] = $v_type;
                    list($r_bill, $r_billdate) = resolve_debtor_receipt($r_p, $conn);
                    $final_rows[] = [
                        'vn'          => $r_p['vn'],
                        'vstdate'     => $r_p['vstdate'],
                        'cid'         => $r_p['cid'],
