...
<style>
  ...
  /* 🚀 ดัน SweetAlert2 (Modal ยืนยัน) ให้แสดงผลอยู่หน้าสุดทับทุก Modal และ Backdrop */
  .swal2-container {
                    ...
                    <form id="upload-formSTM" enctype="multipart/form-data" class="stm-box-general">
                      ...
                      <div class="stm-box-header">
                        ...
                        <div>
                          <span class="stm-pill-badge-teal">
                            <i class='bx bxs-file-blank text-success'></i> รองรับไฟล์ Excel (.xlsx, .xls) และ ZIP (.zip)
                          </span>
                        </div>
                      </div>
                      ...
                      <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-8">
                          ...
                          <input class="form-control stm-file-control" name="excel_file" type="file" id="formFile" accept=".xlsx,.xls,.zip" required />
                        </div>
                      </div>
...
<script src="https://code.iconify.design/iconify-icon/1.0.7/iconify-icon.min.js"></script>
    ...
    <script>
      ...
      $("#upload-formSTM").on('submit', function(e) {
            ...
            // Smart Guard: ตรวจสอบกรณีผู้ใช้นำไฟล์ฟอกไต (DCKD / LGO-HD / COCD / SOCD) มาใส่ในการ์ดนำเข้า STM ทั่วไป
            var isDialysisFile = (fileName.indexOf('DCKD') !== -1) || 
                                 (fileName.indexOf('LGO-HD') !== -1) || 
                                 (fileName.indexOf('COCD') !== -1) || 
                                 (fileName.indexOf('SOCD') !== -1);
            if (isDialysisFile) {
              Swal.fire({
                icon: 'warning',
                title: 'ตรวจพบไฟล์ STM ฟอกไต (DCKD/HD)',
                html: 'ไฟล์ <b>' + fileInput.files[0].name + '</b> เป็นไฟล์ชดเชยค่าบริการฟอกเลือดล้างไต<br><br>กรุณานำเข้าที่การ์ด <b>"นำเข้า STM ฟอกไต (นำเข้าข้อมูลไฟล์ Excel / XML / ZIP)"</b> ที่อยู่ด้านล่าง',
                confirmButtonText: 'รับทราบและไปที่การ์ดฟอกไต'
              }).then(() => {
                $('html, body').animate({
                  scrollTop: $("#upload-formCKD").offset().top - 100
                }, 500);
              });
              return false;
            }

            // นำเข้าไฟล์ STM ทั่วไปทุกกองทุน (UCS, OFC, LGO, BKK, SRT, SSS, BMT) ผ่านระบบพรีวิวและตรวจสอบความถูกต้อง
            ...
            Swal.fire({
              ...
              html: '<p class="mb-2">ระบบกำลังอ่านข้อมูลและตรวจสอบโครงสร้างไฟล์ Statement (' + fund + ')</p><p class="text-muted small">จำแนกหมวดเงินชดเชย และจำลองจับคู่ผังลูกหนี้ในหน่วยความจำ...</p>',
      ...
      $("#upload-formCKD").on('submit', function(e) {
            ...
            var fileInputCkd = document.getElementById('formFile2');
            if (fileInputCkd && fileInputCkd.files && fileInputCkd.files.length) {
              for (var i = 0; i < fileInputCkd.files.length; i++) {
                var ckdFileName = fileInputCkd.files[i].name.toUpperCase();
                if ((ckdFileName.indexOf('OPUCS') !== -1) || (ckdFileName.indexOf('IPUCS') !== -1) || (ckdFileName.indexOf('ECLAIM_') !== -1 && ckdFileName.indexOf('HD') === -1)) {
                  Swal.fire({
                    icon: 'warning',
                    title: 'ตรวจพบไฟล์ STM ทั่วไป',
                    html: 'ไฟล์ <b>' + fileInputCkd.files[i].name + '</b> เป็นไฟล์ Statement ผู้ป่วยนอก/ในทั่วไป<br><br>กรุณานำเข้าที่การ์ด <b>"นำเข้า STM (นำเข้าข้อมูลไฟล์ Excel)"</b> ด้านบน',
                    confirmButtonText: 'รับทราบ'
                  });
                  return false;
                }
