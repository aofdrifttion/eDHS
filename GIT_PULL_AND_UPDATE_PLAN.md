...
| **Step 7** | ยกระดับ Auto-Migration ตรวจสอบครบทุกคอลัมน์ (แก้ Unknown column 'accountcode'), ปลดล็อกสิทธิ์อัปเดตสากล, เปิดปุ่มแนบใบเสร็จทุกสิทธิ์ใน VlookUP และติดตั้งปุ่มรีอัปเดต | ผู้พัฒนา | ✅ เสร็จสิ้น (v21.3.09.69) |
| **Step 8** | แก้ไขปัญหาเลขที่ใบเสร็จผังแม่สูญหายเมื่อตัดลูกหนี้ CR/SSS และปรับปรุงยอดเงินชดเชยผังแม่ | ผู้พัฒนา | ✅ เสร็จสิ้น (v24.09.69) |
| **Step 9** | ปรับปรุงระบบทะเบียนคุมอายุลูกหนี้ (ทะบียนคุมอายุลูกหนี้1.php) รองรับการตัดโอน CR/SSS พร้อมสร้างระบบ Drill-Down รายบุคคล (get_debtor_detail.php) และ Multi-Source Receipt Resolution ยันยอดตรง 100% | ผู้พัฒนา | ✅ เสร็จสิ้น (v25.09.69) |
| **Step 10** | รพ. ปลายทาง ดึงอัปเดตผ่าน `update.bat` / `update.sh` หรือกดปุ่ม 1-Click Update บนหน้าเว็บ eDHS | รพ. ปลายทาง | พร้อมใช้งาน 🚀 |
...
## 8. สถาปัตยกรรมการแก้ไขโครงสร้างข้ามโรงพยาบาลและการรีอัปเดตระบบ (v21.3.09.69)

1. **การแก้ไข Error 'Unknown column accountcode' ด้วย Full Column Auto-Migration:**
   - ในโรงพยาบาลบางแห่งที่เคยมีการสร้างตาราง `imr_tb_payment_history` ในอดีตก่อนมีคอลัมน์ `accountcode` คำสั่ง `CREATE TABLE IF NOT EXISTS` จะไม่ทำการเพิ่มคอลัมน์ใหม่ให้ ส่งผลให้เกิด Error เมื่อบันทึกการแบ่งจ่าย
   - ฟังก์ชัน `ensure_payment_history_table()` ใน `db_helper.php` จึงได้รับการปรับปรุงให้รัน `SHOW COLUMNS FROM imr_tb_payment_history` และตรวจสอบครบทุกฟิลด์ (`accountcode`, `bill_date`, `patient_type`, `bill_no`, `note`) พร้อมสร้างดัชนี `idx_accountcode` และ `idx_ref_vn_an` ให้อัตโนมัติทันที
2. **เปิดสิทธิ์การอัปเดตระดับสากล (Universal Update Access):**
   - ปลดล็อกข้อจำกัด Session ใน `api_update.php` และ `enforce_mandatory_system_update()` เพื่อให้เจ้าหน้าที่ทุกคนของโรงพยาบาลสามารถกดอัปเดตระบบได้ทันทีโดยไม่ต้องรอแอดมินล็อกอิน
3. **เปิดปุ่มแนบใบเสร็จในหน้าต่าง Modal VlookUP.php:**
   - ปลดล็อกการเช็คเงื่อนไข `in_array($_SESSION['role'], ['finance', 'admin'])` ที่ปุ่มแนบใบเสร็จ (Statement REP NO และ DCKD) ทำให้ผู้ใช้งานทุกระดับสามารถแนบใบเสร็จได้ตามที่ต้องการ
4. **ยุทธศาสตร์การจัดการเวอร์ชันและการรีอัปเดต (Version Increment & Force Re-update):**
   - **กรณีเครื่องที่อัปเดต v21.2.09.69 ไปแล้ว:** การปล่อยเวอร์ชันใหม่เป็น **`v21.3.09.69`** จะทำให้เครื่องเหล่านั้นตรวจพบเวอร์ชันใหม่ทันที และกดอัปเดตเพื่อรับการแก้ปัญหาโครงสร้างฐานข้อมูลและปุ่มแนบใบเสร็จได้อย่างอัตโนมัติ
   - **กรณีเครื่องที่ยังไม่ได้อัปเดต (v21.1 หรือเก่ากว่า):** จะสามารถอัปเดตข้ามขั้นสู่ `v21.3.09.69` ได้โดยตรงในคลิกเดียว
   - **ปุ่มรีอัปเดต (Force Re-update):** ติดตั้งปุ่ม "รีอัปเดต" ในหน้าต่าง Changelog Modal และรองรับพารามิเตอร์ `force=1` ใน `api_update.php` เพื่อให้ผู้ดูแลสามารถสั่งดึงโค้ดและลงทับซ้ำได้ตลอดเวลาแม้จะอยู่บนเวอร์ชันล่าสุดแล้ว

---

## 9. สถาปัตยกรรมการคงเลขที่ใบเสร็จผังแม่เมื่อตัดแบ่งลูกหนี้ CR/SSS (v24.09.69)

1. **การแยกแยะเลขที่ใบเสร็จระหว่างผังแม่และผังลูก (Targeted Receipt Isolation):**
   - **ปัญหาเดิม:** ในฟังก์ชันแสดงผลรายการลูกหนี้ของ `system/datatimestamp-insert2.php` (ทั้ง 6 ฟังก์ชัน: `actionopdlist`, `actionipdlist`, `actionopdlistonly`, `actionipdlistonly`, `actionopdlistonlypp`, `actionipdlistonlypp`) และ `system/ลูกหนี้รายตัว2.php` มีเงื่อนไขคำสั่ง `$row['bill'] = ''; $row['billdate'] = '';` แบบเหมารวมเมื่อ `$mother_comp <= 0` ส่งผลให้เคสที่มีการตัดค่าใช้จ่ายเข้าผังลูก (เช่น CR `.216`, `.217` หรือ SSS `.309`, `.310`) แม้ผู้ใช้จะพิมพ์บันทึกเลขที่ใบเสร็จและวันที่ออกใบเสร็จของผังแม่จริงลงไป ระบบก็จะล้างค่าทิ้งกลายเป็นค่าว่างทันที ทำให้ไม่สามารถใส่เลขใบเสร็จในผังแม่ได้
   - **การแก้ไข:** ปรับเปลี่ยนเงื่อนไขเป็นการตรวจสอบแบบเจาะจง:
     ```php
     ...
     ```
     ระบบจะล้างค่าเฉพาะกรณีที่เลขที่ใบเสร็จของผังแม่ตรงกับเลขที่ใบเสร็จของผังลูก และผังแม่ยังไม่ได้ปิดยอดจริงเท่านั้น หากผู้ใช้บันทึกเลขที่ใบเสร็จเฉพาะของผังแม่เอง ระบบจะคงค่าไว้และแสดงผลอย่างถูกต้อง 100%
2. **การคงค่าใบเสร็จในรายงานใบลูกหนี้รายตัวสำหรับพิมพ์ (`system/ลูกหนี้รายตัว2.php`):**
   - ประยุกต์ใช้กฎ Targeted Receipt Isolation แบบเดียวกันกับตารางแสดงผลในใบลูกหนี้รายตัวสำหรับพิมพ์ ทำให้พิมพ์ใบสรุปลูกหนี้รายตัวแสดงเลขที่ใบเสร็จและวันที่ออกใบเสร็จของผังแม่ตรงตามฐานข้อมูลจริง
3. **การปรับปรุงผลรวมเงินชดเชยผังแม่เมื่อตัดลูกหนี้ CR/SSS (`follow_money Alignment`):**
   - ใน `datatimestamp-insert2.php` (ส่วนบันทึก `follow_money`) ปรับปรุงการคำนวณและตอบกลับยอดรวมเงินชดเชยระหว่างผังแม่และผังลูก เพื่อให้ยอดรวมที่แสดงในตารางสรุปถูกต้องและสอดคล้องกันทุกจุด

---

## 10. สถาปัตยกรรมทะเบียนคุมอายุลูกหนี้และ Drill-Down รายบุคคล (v25.09.69)

1. **การเชื่อมโยงระบบตัดโอน CR/SSS ในทะเบียนคุมอายุลูกหนี้ (`system/ทะบียนคุมอายุลูกหนี้1.php`):**
   - หักยอดโอนออกจากผังแม่ (`.201`, `.202`, `.203`, `.209`, `.301`, etc.) และโอนเข้าผังลูก (`.216`, `.217`, `.309`, `.310`) แบบเรียลไทม์
   - จัดช่วงอายุหนี้ (`< 3 เดือน`, `3-12 เดือน`, `> 12 เดือน`) อ้างอิงตามวันที่รับบริการจริง (`srv_date`)
   - คำนวณอายุหนี้แบบไดนามิกเทียบกับวันสิ้นสุดของงวดเดือนที่เลือก (`$last_day_date`)
   - หักยอดชดเชยและรับชำระ STM พร้อมป้องกันยอดตกหล่นด้วย `getReconParentRecoveries` ยันยอดตรงกับรายงาน Column 12 ครบทั้ง 35 ผังบัญชี 100%
2. **ระบบ Drill-Down Modal เจาะลึกรายบุคคล (`system/get_debtor_detail.php`):**
   - พัฒนากลไกตรวจจับใบเสร็จหลายแหล่ง (`resolve_debtor_receipt()`): ตรวจสอบทั้งจากตาราง Breakdown, ตารางหลัก, Mobile string, และ `imr_tb_payment_history`
   - ผังแม่ที่ยอดหนี้คงเหลือสุทธิเป็น 0.00 บาท (เช่น `.201`) แสดงสถานะเคลียร์ครบถ้วน ไม่แสดงรายการตกค้างผิดผัง
   - ผังลูกแสดงรายการโอนเข้าพร้อมระบุประเภทกลุ่มบริการเฉพาะ (`CR-Instrument`, `CR-palliative`, `CR-walkin`, `CR-Tele`)
   - ออกแบบ UI พร้อมบัตรสรุป KPI Cards 4 มิติ และช่อง Live Client-Side Search ค้นหาด่วนตาม HN, ชื่อผู้ป่วย, หรือใบเสร็จได้ทันที
